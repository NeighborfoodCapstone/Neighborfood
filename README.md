...
문서 최종 갱신: 2026-09-08 (저장소 구조 재확인 반영)
...
> ⚠️ `seed_admin.py`는 2026-09-08 기준 저장소에 존재하지 않는 것으로 확인되었습니다("폴더 구조"의 정합성 안내 참고). 위 명령은 스크립트 재도입 전까지 실행되지 않으며, 관리자 계정이 필요하면 DB에 직접 `role='admin'` 행을 삽입하는 등 임시 수단을 사용해야 합니다.
...
>
> 현재 저장소에 존재하는 자동화 테스트는 `tests/test_receipt_parser_v212.py`(영수증 파서 전용) 하나뿐이며, 관리자 계정 생성·더미 시드·정산 등 나머지 기능은 위 스크립트가 없는 상태이므로 별도 수동 절차로 병행해야 합니다. 아래 "5. 관리자 계정 생성"·"6. 더미 데이터 주입" 안내도 이 상태를 반영합니다.
...
| `/api/admin` | 관리자 | 회원·신고·공지 관리, 채팅 모니터링 |
